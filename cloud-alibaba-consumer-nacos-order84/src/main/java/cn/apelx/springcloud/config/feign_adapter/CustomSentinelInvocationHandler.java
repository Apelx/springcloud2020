...
public class CustomSentinelInvocationHandler implements InvocationHandler {
	...
	private final static Logger log = LoggerFactory.getLogger(CustomSentinelInvocationHandler.class);
			...
			if (methodMetadata == null) {
				...
				try {
					...
					if (fallbackFactory != null) {
						...
						// 若是R类型 执行自动降级返回R
						if (CommonResponse.class == method.getReturnType()) {
							log.error("feign 服务间调用异常", ex);
							return new CommonResponse<>(200, ex.getLocalizedMessage());
						}
						else {
							throw ex;
						}
